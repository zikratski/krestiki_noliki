...
def getKeyboard(callBackData):
    ...
    if callBackData != None:  # если
        ...
        for i in range(0, 3):
            for j in range(0, 3):
                keyboard[i].append(types.InlineKeyboardButton(callBackData[j + i * 3], callback_data=str(j + i * 3) + callBackData))
...
def newGame(update, _):
    ...
    update.message.reply_text(st.ANSW_YOUR_TURN, reply_markup=types.InlineKeyboardMarkup(getKeyboard(data)))
...
def button(update, _):
    ...
    if alert is None:  # если не получен сигнал тревоги (alert==None), то редактируем сообщение и меняем клавиатуру
        ...
        query.edit_message_text(text=message, reply_markup=types.InlineKeyboardMarkup(getKeyboard(callbackData)))
...
#     updater = types.Updater(getToken())  # получения токена из файла 'token.txt' и инициализация updater
#     # добавление обработчиков
#     updater.dispatcher.add_handler(types.CommandHandler('start', newGame))
#     updater.dispatcher.add_handler(types.CommandHandler('new_game', newGame))
